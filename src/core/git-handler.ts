...
import { GitDiffModal, GitCommitModal } from "modals";
	...
	async showAllDiff() {
		...
		setTimeout(async () => {
			try {
				...
				// 获取已跟踪的 .md 文件的差异
				try {
					const { stdout: trackedDiff } = await execPromise(
						'git diff -- "*.md"',
						{ cwd: repoPath },
					);
					if (trackedDiff) {
				...
				try {
					...
					const untrackedFilesContent = untrackedDiffs
						.filter((diff) => diff)
						.join("\n");
						...
						if (vaultPath && fs.existsSync(vaultPath)) {
							try {
								...
								try {
									...
									if (
										vaultCommitError &&
										vaultCommitError.stderr &&
										(vaultCommitError.stderr.includes(
											"nothing to commit",
										) ||
											vaultCommitError.stderr.includes(
												"no changes added to commit",
								...
								new Notice(
									"Obsidian库Git操作失败，请手动提交本仓库的git更新，避免影响下一次的差异翻译功能",
								);
							}
						}

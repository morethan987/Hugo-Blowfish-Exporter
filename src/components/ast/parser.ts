...
export function parseMarkdown(src: string): MarkdownNode {
	...
	while (i < lines.length) {
		...
		if (/^>\s*\[![^\]]+\]/.test(line)) {
			...
			const foldMark = typeMatch ? typeMatch[2] : ""; // 获取 + 或 - 或 ''
			const titleText = typeMatch ? typeMatch[3]!.trim() : "";

			// 计算 auto_fold 状态
			// '-' 代表默认折叠(true)，'+' 代表默认展开(false)，没标记则为 undefined
			let auto_fold: boolean | undefined;
			if (foldMark === "-") {
				auto_fold = true;
			} else if (foldMark === "+") {
				auto_fold = false;
...
function parseInline(text: string): MarkdownNode[] {
	...
	const flush = () => {
		if (buffer) {
			nodes.push({ type: NodeType.Text, value: buffer });
			buffer = "";
		}
	};

...
export const wikiLinkRuleWechat = [
    new RuleBuilder('非展示型wiki链接转换')
        .describe('将非展示型wiki链接转换为对应的Wechat简码')
        ...
        .transform((node, context) => {
            ...
            if(linkType === 'external-heading' || linkType === 'article'){
                WechatLink = `[${alias || heading}]({{< ref "/${context.data.settings.blogPath}/${getSlugByName(context.data.app, file_name)}/${formated_heading ? '#' + formated_heading : ''}" >}})`;
            } else if(linkType === 'internal-heading'){
                WechatLink = `[${alias || heading}]({{< relref "#${formated_heading}" >}})`;
            }
    ...
    new RuleBuilder('展示型wiki链接转换')
        .describe('将展示型wiki链接转换为对应的Wechat简码')
        ...
        .transform((node, context) => {
            ...
            const WechatLink = `{{< mdimporter url="content/${context.data.settings.blogPath}/${context.data.slug}/${fileName}" >}}`;

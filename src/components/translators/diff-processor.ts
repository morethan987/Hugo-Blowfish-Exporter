...
export class DiffProcessor {
	 ...
	 * @returns 更新信息列表
	 */
	async processDiffChanges(
		changes: DiffChange[],
	): Promise<ParagraphUpdate[]> {
		console.debug(
			"🚀 [DiffProcessor] 开始处理差异变更，总数:",
			changes.length,
		);
		console.debug(
			"📊 [DiffProcessor] 输入变更详情:",
		...
		for (let i = 0; i < changes.length; i++) {
			...
			const translatedLines =
				await this.processContentTranslation(addedLines);
			console.debug(
				"✅ [DiffProcessor] 翻译完成，结果:",
				translatedLines,
			);
			...
			if (oldCount === 0 && newCount > 0) {
				...
				update = this.handleDeletedContent(
					newStart,
					oldStart,
					oldCount,
					removedLines,
				...
				update = this.handleModifiedContent(
					oldStart,
					oldCount,
					newCount,
					removedLines,
	 ...
	 * @returns 翻译后的行
	 */
	private async processContentTranslation(
		lines: string[],
	): Promise<string[]> {
		console.debug(
			"🔤 [DiffProcessor] 开始处理内容翻译，行数:",
			lines.length,
		);
		...
		if (toTranslate.length > 0) {
			...
			console.debug(
				"⏭️  [DiffProcessor] 没有需要翻译的内容，全部保持原样",
			);
			// 没有需要翻译的内容，全部保持原样
			lines.forEach((line, i) => {
				translatedLines[i] = line;
